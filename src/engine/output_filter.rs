...
pub(crate) struct PerRequestState {
    ...
    /// Buffer for raw text accumulated while in thinking mode.  Used to detect
    /// `</think>` when it spans multiple BPE tokens (e.g. `</` + `think` + `>`).
...
pub(crate) fn apply_output_filter(state: &mut PerRequestState, raw: &str) -> (String, bool) {
    ...
    if state.in_thinking {
        // Accumulate raw text so we can detect multi-token `</think>` above.
        state.thinking_buf.push_str(raw);
        // Budget enforcement: force-close when the character limit is reached.
